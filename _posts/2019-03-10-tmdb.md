...
```
SELECT cast_id, cast_name, AVG(score) as average_score FROM movies INNER JOIN movie_cast ON id = movie_id GROUP BY cast_id HAVING count(movie_id) > 2 AND score >=50 ORDER BY -average_score LIMIT 10;
```
...
```
SELECT cast_id, cast_name, COUNT(movie_id) as movie_count FROM movie_cast GROUP BY cast_id ORDER BY -movie_count LIMIT 5;
```
###### Creating Views
...
SELECT a.cast_name as cast_member_id1, b.cast_name as cast_member_id2, count(*) as movie_count, ROUND(AVG(score),2) as average_movie_score FROM movie_cast a INNER JOIN movie_cast b ON (a.movie_id = b.movie_id AND a.cast_name < b.cast_name) INNER JOIN movies c on b.movie_id = c.id GROUP BY a.cast_name, b.cast_name HAVING movie_count >=3 AND average_movie_score >= 40 ORDER BY cast_member_id1, cast_member_id2;
```

###### Best collaborators
I list the five cast members with the highest average scores from the good_collaboration view, and call this score the 'collaboration_score'. This score is the average of the average_movie_score corresponding to each cast member, including actors in cast_member_id1 as well as cast_member_id2. The output is sorted in descending order of this score and alphabetically by cast_name in case of a tie in the score. The output is in the below format. 

|cast_id|cast_name|collaboration_score|
```
SELECT cast_id, cast_member_id1 AS cast_name, AVG(average_movie_score) AS collaboration_score FROM good_collaboration INNER JOIN movie_cast ON cast_member_id1 = cast_name GROUP By cast_name ORDER BY -collaboration_score LIMIT 5; 
```
...
|   id   | name | score |
```
SELECT id,name,score FROM movies ORDER BY -score THEN BY name LIMIT 7;
```

###### Calcualting a proportion
I found the proportion of movies having a score > 50.
```
SELECT ((SUM(CASE when score > 50 then 1 else 0 end)*100)/COUNT(*)) AS prop FROM movies;
```
